...
<body>
	<div style="position:fixed;display:none" id="mask"></div>
	<div style="position:fixed;display:none" id="cartAddWarn"><div>Veuillez cochez votre sauce et indiquez le nombre de baguette qu'il vous faut  dans " mon panier". <br><br><button id="goToCart" class="btn btn-warning">Allez à panier</button> <button class="btn btn-info cancel">Bon, Je sera choisirai</button></div></div>
	<div style="position:fixed;display:none" id="cartEmptyWarn"><div>Vous ne choisissez pas n'importe quel produits<br><br><button class="cancel btn btn-danger">Allez à choisissez</button></div></div>
				...
				<table class='cart_class'>
					...
					<tr id="number_change">
						...
						<td colspan="2" id="isEmporter">
							<?php 
								if ($isEmporter) {
									echo "<span value='true' onClick='setEmporter(false)'><i class='fa fa-toggle-on'></i></span>";
								}else{
									echo "<span value='false' onClick='setEmporter(true)'><i class='fa fa-toggle-off'></i></span>";
								}
							?>
						</td>
					</tr>
					<tr class="tr_special" id="p701">
						<td>Paire De Baguettes</td>
						<td class="opacity0">Graduit</td>
						<td id="bagu_id"><!--
							<span onclick="updateToCart(701,<?php 
								if ($p_de_bagu_qty == 0) {
									echo 1;
								}else{
									echo 0;
							...
							?></span>-->
							<span class="minusCart" onclick='minusCart(701, <?=$p_de_bagu_qty;?>)'>
								<i class="fa fa-minus"></i>
							</span>
							<span>&nbsp;&nbsp;<?=$p_de_bagu_qty?>&nbsp;&nbsp;</span>
							<span class="plusCart" onclick='plusCart(701, <?=$p_de_bagu_qty?>)'>
								<i class="fa fa-plus"></i>
							</span>
						</td>
						<td><div class="delete_cart_menu" onclick="deleteCart(701)" name="701"></div></td>
					</tr>
					<tr class="tr_special" id="p702">
						<td>Sauce Sucre</td>
						<td class="opacity0">Graduit</td>
						<td id="sucre_id">
							<span onclick="updateToCart(702,<?php 
								if ($sucre_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="minusCart(702)" name="702"></div></td>
					</tr>
					<tr class="tr_special" id="p703">
						<td>Sauce Sale</td>
						<td class="opacity0">Graduit</td>
						<td id="sale_id">
							<span onclick="updateToCart(703,<?php 
								if ($sale_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(703)" name="703"></div></td>
					</tr>
					<!--
					<tr class="tr_special" id="p704">
						<td>Gingembre</td>
						<td class="opacity0">Graduit</td>
						<td id="gin_id">
							<span onclick="updateToCart(704,<?php 
								if ($gingembre_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(704)" name="704"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					<tr class="tr_special" id="p705">
						<td>Wasabi</td>
						<td class="opacity0">Graduit</td>
						<td id="was_id">
							<span onclick="updateToCart(705,<?php 
								if ($wasabi_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(705)" name="705"><i class="fa fa-trash-o"></i></div></td>
					</tr>-->
				</table>
				<div class="clear"></div>
				<?php foreach ($totals as $total) {?>
					<div class="row float_right cartTotal">
						<div class="col-md-4 font_big"><?php echo $total['title'];?></div>
						<div class="col-md-8 font_big" id="money_in_total"><?php echo $total['text'];?></div>
					</div>
				<?php }?>
						...
						<div class="row" id="cate_row_restr_width_2">
							<div class="navbar-header">
								...
								<div class="login-suc-div" id="not_home_login" >
										<ul id="loginornot" class="mainNav" style="padding-top:18px;">
											<?php if (!$logged) { ?>
							...
							<div class="collapse navbar-collapse">
								<ul class="nav navbar-nav navbar-right mainNav" id="mainNav" style="padding-top:23px;">
