...
class ControllerCatalogProduct extends Controller {
	private $error = array(); 
  	...
  	protected function getForm() {
		...
		//显示套餐通道
		if(isset($this->request->post['setmeal'])){
			$this->data['setmeals'] = $this->request->post['setmeal'];
		}elseif(!empty($product_info['setmeal'])){
			$this->data['setmeals'] = $product_info['setmeal'];
		}else{
			$this->data['setmeals'] = '';
		}
		if(!empty($this->data['setmeals'])){
			$this->data['setmeals'] = explode(',',$this->data['setmeals']);
		}
	    ...
	    //meal
		if (isset($this->request->post['product_meal'])) {
			$product_meals = $this->request->post['product_meal'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_meals = $this->model_catalog_product->getProductMeals($this->request->get['product_id']);
		} else {
			$product_meals = array();
		}
		if (!empty($product_meals)) {
        	foreach ($product_meals as $key => $value) {
        		if ($value['setmeal']) {
        			$product_meals[$key]['setmeal'] = explode(',',$value['setmeal']);
        		}
        	}
        }	
        //echo "<pre>";print_r($product_meals);die;echo "</pre>";die;
		$this->data['product_meals'] = $product_meals;
		// Attributes
