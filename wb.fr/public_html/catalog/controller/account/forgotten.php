...
class ControllerAccountForgotten extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$mail->Username = 'jichangfeng67@gmail.com';                 // SMTP username
			$mail->Password = 'As13849672026!@';                           // SMTP password
			$mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
			$mail->Port = 587;                                    // TCP port to connect to

			$mail->From = 'no-reply@wasabiresto.fr';
			$mail->FromName = 'wasabiresto';
			$mail->addAddress($this->request->post['email']);     // Add a recipient
			//$mail->addAddress('ellen@example.com');               // Name is optional
			$mail->addReplyTo('info@wasabiresto.fr', 'Information');
			...
			if(!$mail->send()) {
			    ...
			    //session_start();
			    $_SESSION["mailMessage"] = "no";
			} else {
			    echo 'Message has been sent';
			    //session_start();
			    $_SESSION["mailMessage"] = $this->request->post['email'];
			}
			/*
			$mail = new Mail();
