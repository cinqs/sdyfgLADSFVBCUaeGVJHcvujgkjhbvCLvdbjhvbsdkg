...
// NEW
    	if (isset($this->request->post['shipping_batiment'])) {
      		$this->data['shipping_batiment'] = $this->request->post['shipping_batiment'];
    	} elseif (!empty($order_info)) { 
			$this->data['shipping_batiment'] = $order_info['shipping_batiment'];
		} else {
      		$this->data['shipping_batiment'] = '';
    	}

    	if (isset($this->request->post['shipping_escalier'])) {
      		$this->data['shipping_escalier'] = $this->request->post['shipping_escalier'];
    	} elseif (!empty($order_info)) { 
			$this->data['shipping_escalier'] = $order_info['shipping_escalier'];
		} else {
      		$this->data['shipping_escalier'] = '';
    	}

    	if (isset($this->request->post['shipping_interphone'])) {
      		$this->data['shipping_interphone'] = $this->request->post['shipping_interphone'];
    	} elseif (!empty($order_info)) { 
			$this->data['shipping_interphone'] = $order_info['shipping_interphone'];
		} else {
      		$this->data['shipping_interphone'] = '';
    	}

    	if (isset($this->request->post['shipping_ascenceur'])) {
      		$this->data['shipping_ascenceur'] = $this->request->post['shipping_ascenceur'];
    	} elseif (!empty($order_info)) { 
			$this->data['shipping_ascenceur'] = $order_info['shipping_ascenceur'];
		} else {
      		$this->data['shipping_ascenceur'] = '';
    	}

    	if (isset($this->request->post['shipping_digicode1'])) {
      		$this->data['shipping_digicode1'] = $this->request->post['shipping_digicode1'];
    	} elseif (!empty($order_info)) { 
			$this->data['shipping_digicode1'] = $order_info['shipping_digicode1'];
		} else {
      		$this->data['shipping_digicode1'] = '';
    	}

    	if (isset($this->request->post['shipping_digicode2'])) {
      		$this->data['shipping_digicode2'] = $this->request->post['shipping_digicode2'];
    	} elseif (!empty($order_info)) { 
			$this->data['shipping_digicode2'] = $order_info['shipping_digicode2'];
		} else {
      		$this->data['shipping_digicode2'] = '';
    	}

    	if (isset($this->request->post['shipping_etage'])) {
      		$this->data['shipping_etage'] = $this->request->post['shipping_etage'];
    	} elseif (!empty($order_info)) { 
			$this->data['shipping_etage'] = $order_info['shipping_etage'];
		} else {
      		$this->data['shipping_etage'] = '';
    	}

    	if (isset($this->request->post['shipping_porte'])) {
      		$this->data['shipping_porte'] = $this->request->post['shipping_porte'];
    	} elseif (!empty($order_info)) { 
			$this->data['shipping_porte'] = $order_info['shipping_porte'];
		} else {
      		$this->data['shipping_porte'] = '';
    	}
    	...
    	if (isset($this->request->post['liv'])) {
      		$this->data['liv'] = $this->request->post['liv'];
    	} elseif (!empty($order_info)) { 
			$this->data['liv'] = $order_info['liv'];
		} else {
      		$this->data['liv'] = '';
    	}

    	if (isset($this->request->post['couv'])) {
      		$this->data['couv'] = $this->request->post['couv'];
    	} elseif (!empty($order_info)) { 
			$this->data['couv'] = $order_info['couv'];
		} else {
      		$this->data['couv'] = '';
    	}
//************    	  	
		...
		if(!empty($order_products)){
			foreach ($order_products as $key_product => $value_product) {	
				if(!empty($value_product['ordermeals'])){
					 $order_products[$key_product]['meals'] = $this->model_sale_order->getOrderMeals($value_product['ordermeals']);

				}
			}
		}

		//echo "<pre>";print_r($meals->rows);echo "</pre>";exit;
		...
		//echo "<pre>";print_r($this->data['order_totals']);die;
	...
	public function info() {
		...
		if ($order_info) {
			$this->language->load('sale/order');
		    if($order_info['prompt'] ==1){
		    	$this->model_sale_order->updatePrompt($order_id,0);
		    }
			$this->document->setTitle($this->language->get('heading_title'));
			$this->data['fulls']=array();
			$this->data['fulls']=$this->model_sale_order->getFull($order_info['full']);
			...
			$this->data['text_batiment'] = $this->language->get('text_batiment');
			$this->data['text_escalier'] = $this->language->get('text_escalier');
			$this->data['text_interphone'] = $this->language->get('text_interphone');
			$this->data['text_ascenceur'] = $this->language->get('text_ascenceur');
			$this->data['text_digicode1'] = $this->language->get('text_digicode1');
			$this->data['text_digicode2'] = $this->language->get('text_digicode2');
			$this->data['text_etage'] = $this->language->get('text_etage');
			$this->data['text_porte'] = $this->language->get('text_porte');
			$this->data['text_description'] = $this->language->get('text_description');
			$this->data['text_liv'] = $this->language->get('text_liv');
			$this->data['text_couv'] = $this->language->get('text_couv');
			...
			foreach ($products as $product) {
				$meal_data = array();
				if(!empty($product['ordermeals'])){
						 $meal_data = $this->model_sale_order->getOrderMeals($product['ordermeals']);
				}
		    ...
		    //echo '<pre>';print_r($this->data['products']);echo "</pre>";die;
			...
			//print_r(array_keys($this->data['totals'][0]));
			//echo $this->data['totals'][1];
			...
			//echo "<pre>";print_r('<meta charset="UTF-8" />');echo "</pre>";
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$replace = array(
					...
					'batiment' => $order_info['shipping_batiment'],
					'escalier' => $order_info['shipping_escalier'],
					'interphone' => $order_info['shipping_interphone'],
					'ascenceur' => $order_info['shipping_ascenceur'],
					'digicode1' => $order_info['shipping_digicode1'],
					'digicode2' => $order_info['shipping_digicode2'],
					'etage' => $order_info['shipping_etage'],
					'porte' => $order_info['shipping_porte'],
					'description' => $order_info['shipping_description'],
					'liv' => $order_info['liv'],
					'couv' => $order_info['couv'],
				...
				$this->data['orders'][] = array(
					...
					'total'              => $total_data,
			    	'liv'                => nl2br($order_info['liv']),
				    'couv'               => nl2br($order_info['couv']),
