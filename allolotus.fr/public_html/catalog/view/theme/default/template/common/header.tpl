...
<head>
	...
	<meta name="keywords" content="plat chinoise, livraison plat chinoise, livraison, livraison gratuit, allolotus, alloresto, 75010, 75018, 75019, 75002, 75003 ,75009, 75001, 75004, 75011">
	<meta name="description" content="restaurant chinoise a Paris">
	<meta name="author" content="EYELLE-PARIS http://eyelle-paris.com">
...
<body>
	...
	<div id="fixedCart">
		<h5 class="bold font16">Livraison  gratuite à partir de 15€</h5>
				...
				<table class='cart_class'>
					...
					<?php foreach ($productss as $product) {?>
						<?php //判断是否是sucre
							...
							}elseif ($product['key'] == 609) {//判断是否是sale
								$sale_qty = $product['quantity'];
							}elseif ($product['key'] == 610) {
								$gingembre_qty = $product['quantity'];
							}elseif ($product['key'] == 607) {
								$p_de_bagu_qty = $product['quantity'];
							}elseif ($product['key'] == 611) {
					...
					<tr class="tr_special">
						...
						<td id="bagu_id">
							<span onclick="updateToCart(607,<?php 
								if ($p_de_bagu_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(607)" name="607"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					<tr class="tr_special">
						<td>Sauce Sucre</td>
						<td class="opacity0">Graduit</td>
						<td id="sucre_id">
							<span onclick="updateToCart(608,<?php 
								if ($sucre_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="minusCart(608)" name="608"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					<tr class="tr_special">
						<td>Sauce Sale</td>
						<td class="opacity0">Graduit</td>
						<td id="sale_id">
							<span onclick="updateToCart(609,<?php 
								if ($sale_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(609)" name="609"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					...
					<tr class="tr_special">
						...
						<td class="opacity0">Graduit</td>
						<td id="gin_id">
							<span onclick="updateToCart(610,<?php 
								if ($gingembre_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(610)" name="610"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					<tr class="tr_special">
						<td>Wasabi</td>
						<td class="opacity0">Graduit</td>
						<td id="was_id">
							<span onclick="updateToCart(611,<?php 
								if ($wasabi_qty == 0) {
									echo 1;
								}else{
									echo 0;
						...
						<td><div class="delete_cart_menu" onclick="deleteCart(611)" name="611"><i class="fa fa-trash-o"></i></div></td>
					</tr>
					<tr>
						<td colspan="2">Couvert</td>
						<td colspan="2">
							<span onclick='couvUpdate(<?php echo intval($couv)-1;?>)' id='couvMinus'>
								<i class='fa fa-minus'></i>
							</span>
							<span id='couv'> <?=$couv?> </span>
							<span onclick='couvUpdate(<?php echo intval($couv)+1;?>)' id='couvPlus'>
											...
											foreach ($category["children"] as $child) {
												...
												//这是为了控制甜咸的目录不显示的
													if ($child["category_id"] == $child_cat_id) {
														echo "<a href='".$child["href"]."' class='cur'>".$child['name']."</a><br>";
													}else{
