...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			if(!empty($query->row['setmeal'])){
				$setmeal_arr = explode(',',$query->row['setmeal']);
				$meals = $this->db->query("SELECT * FROM " . DB_PREFIX ."meal WHERE product_id ='".(int)$product_id."' order by meal_id ASC");
				foreach ($meals->rows as $key =>$value) {
					$meals->rows[$key]['setmeal'] = explode(',',$value['setmeal']);
				}

			}

			$temp=$this->db->query("SELECT p.category_id,c.category_id,c.temp FROM " . DB_PREFIX . "product_to_category AS p LEFT JOIN " . DB_PREFIX . "category 	AS c  ON p.category_id=c.category_id WHERE p.product_id='".$query->row['product_id']."'"); 
			return array(
				...
				'temp'             =>  isset($temp->row['temp']) ? $temp->row['temp'] : 0,
				'setmeal'          =>isset($query->row['setmeal']) ? $query->row['setmeal'] : '',
				'meals'            =>isset($meals->rows) ? $meals->rows : ''
			);
		} else {
			return false;
		}
	...
	public function getProducts($data = array()) {
		...
		if (isset($data['start']) || isset($data['limit'])) {
			...
			//limit the quantity of the query back products, would would be effective, but useless.
		...
		if(!empty($data['filter_category_id'])) {
			$category = $this->db->query("SELECT * FROM " . DB_PREFIX . "category WHERE category_id ='".$data['filter_category_id']."'");
			$category_temp=$category->row['temp'];
		}else{
			$category_temp="";
		}
		foreach ($query->rows as $result) {
			$product_data[$result['product_id']] = $this->getProduct($result['product_id']);
			$product_data[$result['product_id']]['temp'] = $category_temp;
	...
	public function nextProduct($product_id){
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product WHERE  status = '1' AND product_id > $product_id  ORDER BY product_id ASC limit 1") ;
		 return $query->row;
	}
	public function prevProduct($product_id){
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product WHERE  status = '1' AND product_id < $product_id  ORDER BY product_id ASC limit 1") ;
		 return $query->row;

	}
	public function  getDisponible($temp){
		...
		if($temp == 1){
			if($t >= intval($settime_am[0]) && $t <= intval($settime_am[1])){
				$json="true";						
			}
		}elseif($temp == 2){
			if($t >= intval($settime_pm[0]) && $t <= intval($settime_pm[1])){
				$json="true";
			}
		}elseif($temp == 3){
