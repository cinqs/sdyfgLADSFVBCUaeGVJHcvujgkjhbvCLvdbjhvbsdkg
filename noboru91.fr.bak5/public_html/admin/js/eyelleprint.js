	...
	***************************************************************************/ 
	...
	function printNihao(data){
		...
		if (data != null) {
			...
			qz.appendImage("http://wasabisushi.fr/admin/img/wasabisushi.png", "ESCP", "single");
				//qz.append(chr(27)+chr(33)+chr(0));

			window["qzDoneAppending"] = function() {
				qz.append(chr(27)+chr(97)+chr(1));
				qz.append('\n'+data[0].store_name+'\n');
				qz.append('06 63 96 75 94 \n');
				qz.append(chr(27)+chr(97)+chr(48)+'\n\n');
	            for(var i = 0; i < data.length; i++){
	            	if (i == 0) {
	            		//qz.append(chr(27)+chr(69)+chr(10));
	            		qz.append('Date    \t'+data[0].date_added+'\n\n');
	            		qz.append('ORDER ID\t'+data[0].order_id+'\n\n');
	            		qz.append('PRENOM  \t'+data[0].firstname+'\n');
	            		qz.append('NOM     \t'+data[0].lastname+'\n\n');
	            		qz.append('TEL     \t'+data[0].telephone+'\n\n');
	            		qz.append('PAYE    \t'+data[0].payment_method+'\n\n');
	            	...
	            	if (i != 0) {
	            		for(var j = 0; j < data[i].length; j++){
	            			if (data[i][j].name.length > 27) {
	            				qz.append(data[i][j].quantity+' X '+data[i][j].name+'\n\t\t\t\t'+parseFloat(data[i][j].total)+'\n');
	            				qz.append('------------------------------------------\n');
	            			}else if (data[i][j].name.length > 19) {
	            				qz.append(data[i][j].quantity+' X '+data[i][j].name+'\t'+parseFloat(data[i][j].total)+'\n');
	            				qz.append('------------------------------------------\n');
	            			}else if (data[i][j].name.length > 11) {
	            				qz.append(data[i][j].quantity+' X '+data[i][j].name+'\t\t'+parseFloat(data[i][j].total)+'\n');
	            				qz.append('------------------------------------------\n');
	            			}else if (data[i][j].name.length > 3) {
	            				qz.append(data[i][j].quantity+' X '+data[i][j].name+'\t\t\t'+parseFloat(data[i][j].total)+'\n');
	            				qz.append('------------------------------------------\n');
	            			}else{
	            				qz.append(data[i][j].quantity+' X '+data[i][j].name+'\t\t\t\t'+parseFloat(data[i][j].total)+'\n');
