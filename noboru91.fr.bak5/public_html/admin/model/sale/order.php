...
class ModelSaleOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			$user = $this->db->query("SELECT *  FROM " . DB_PREFIX . "customer WHERE customer_id='".$order_query->row['customer_id']."' ");
			...
			return array(
				...
				'shipping_batiment'       => $user->row['batiment'],
				'shipping_escalier'       => $user->row['escalier'],
				'shipping_interphone'     => $user->row['interphone'],
				'shipping_ascenceur'      => $user->row['ascenceur'],
				'shipping_digicode1'       =>$user->row['digicode1'],
				'shipping_digicode2'       =>$user->row['digicode2'],
				'shipping_etage'          => $user->row['etage'],
				'shipping_porte'          => $user->row['porte'],
				'shipping_description'    => $user->row['description'],
				'liv'                     => $order_query->row['liv'],
				'couv'                    => $order_query->row['couv'],
				...
				'full'                    => $order_query->row['full'],
				'prompt'                  => $order_query->row['prompt']
			);
		} else {
			return false;
		}
	...
	public function updatePrompt($order_id,$number){

		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET prompt = '" . (int)$number . "' WHERE order_id = '" . (int)$order_id . "'");
	...
	public function getOrderMeals($meals){
		if(empty($meals)){
			return array();
		}
		$mealsArr = explode(',',$meals);
		$count_meal = array_count_values($mealsArr);
		$query = $this->db->query("SELECT * FROM " .DB_PREFIX ."meal WHERE meal_id in($meals)");
		//echo '<pre>';print_r($query->rows);echo "</pre>";die;
		foreach ($query->rows as $key => $value) {
			if($count_meal[$value['meal_id']] > 1){
				for($i = 1;$i < $count_meal[$value['meal_id']];$i++){
					 array_push($query->rows,$value);
				}
			}
		}
		return $query->rows;
	...
	public function getFull($da=0){
		$promotion_1=$this->config->get('config_promotion_1');
		$promotion_2=$this->config->get('config_promotion_2');
		$promotion_3=$this->config->get('config_promotion_3');
		$money_1    =$this->config->get('config_money_1');
		$money_2    =$this->config->get('config_money_2');
		$money_3    =$this->config->get('config_money_3');
		$data['full']=array();
		if($promotion_1 !="" && $money_1 !=""){
			$data['full'][$money_1]=array($promotion_1,$money_1);
		}
		if($promotion_2 !="" && $money_2 !=""){
			$data['full'][$money_2]=array($promotion_2,$money_2);
		}
		if($promotion_3 !="" && $money_3 !=""){
			$data['full'][$money_3]=array($promotion_3,$money_3);
		}
	
		if($da){
			if($da ==$money_1 || $da==$money_2 || $da==$money_3){
